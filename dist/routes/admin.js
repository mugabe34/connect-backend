...
router.post("/products/:id/feature", guard, async (req, res) => {
    ...
    if (!product)
        return res.status(404).json({ message: "Not found" });
    try {
        await Notification_1.default.create({ user: product.seller, type: 'featured', data: { productId: product.id, title: product.title } });
    }
    catch (e) {
        console.error('Failed to create feature notification', e);
    }
    res.json(product);
});
